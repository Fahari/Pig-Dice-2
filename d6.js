...
function D6Animator(id, baseUrl, key) {
  ...
  if (document.getElementById) {
    ...
    if (!targetElem) {
      this.nope(
        "Debug: The tag with specified id (" +
          id +
...
D6Animator.getImageBank = function(key, baseUrl) {
  ...
  if (!imageBank) {
    ...
    if (typeof baseUrl != "string") baseUrl = D6Animator.baseUrl;
...
D6Animator.prototype.setCallback = function(callback, args) {
  if (typeof callback != "function")
    return this.nope(
      "Debug: The first argument to the setCallback method of the D6Animator class must be of type 'function'."
    );
...
D6Animator.prototype.animate = function(sequence, state) {
  if (!sequence)
    return this.nope(
      "Debug: The sequence passed to the animate method of D6Animator was invalid."
    );
  ...
  if (!numNumbers || numNumbers < 1)
    return this.nope(
      "Debug: The sequence passed to the animate method of D6Animator contained no values."
    );
  ...
  if (state == "die") {
    if (numNumbers == 1) {
      window.setTimeout(
        "D6Animator.callAnimatorCallback('" + this.id + "')",
        this.interval
      );
      return true;
    }
...
D6Animator.callAnimatorCallback = function(id) {
  ...
  if (
    ...
    typeof animator.callback == "function"
  )
...
D6Animator.animate = function(id, sequence, state) {
  ...
  if (
    animator &&
    typeof animator == "object" &&
    typeof animator.animate == "function"
  )
...
D6Animator.prototype.showImg = function(number, state) {
  ...
  if (this.useImages) {
    ...
    if (!dieObj)
      return this.nope(
        "Debug: The specified number (" +
          number +
          ") is not a valid number for the D6Animator."
      );
    var dieImg = dieObj[state];
    if (!dieImg)
      return this.nope(
        "Debug: The specified state (" +
          state +
          ") is not a valid state for the D6Animator."
      );
...
function D6AnimGroup(id, animators, isSequenced) {
  // The animators argument is an array of D6Animator and/or D6AnimGroup objects.
  if (typeof id != "string" || !id) return; // allows a dummy object to be created without causing errors below.
  this.id = id;
...
D6AnimGroup.prototype.setCallback = function(callback, args) {
  if (typeof callback != "function")
    return alert(
      "Debug: The first argument to the setCallback method of the D6AnimGroup class must be of type 'function'."
    );
  this.callback = callback;
  this.args = args;
};
...
D6AnimGroup.prototype.notify = function(args) {
  ...
  var numNonZero = 0;
  for (var i = 0; i < this.length; ++i) {
    if (this.completions[i]) numNonZero++;
  }
...
function D6AnimBuilder(
  ...
  if (!locations || typeof locations != "object" || !locations.length)
    locations = new Array();
...
D6AnimBuilder.prototype.genDiceHtml = function(layout, callback, callbackData) {
  ...
  for (var i = 0; i < layout.length; ++i) {
    ...
    genHtml +=
      "<div id='" + this.id + "_diceGroup_" + i + "' class='diceGroup'>";
...
D6AnimBuilder.prototype.start = function() {
  ...
  if (this.callback) animTopGroup.setCallback(this.callback, this.callbackData);
...
D6.dice = function(numDice, callback, callbackData, useImages, buttonLabel) {
  ...
  var genHtml =
    "<div id='diceall'>" +
    builder.genDiceHtml(layout, D6.middleManCallback, middleManData);
  if (buttonLabel != "none") {
    genHtml +=
      "<div id='diceform'><form><input type='button' id='dicebutton' class='btn-roll' value='" +
      buttonLabel +
      '\' onclick=\'D6AnimBuilder.get("dice").reset(); D6AnimBuilder.get("dice").start()\' /></form></div>';
  }
  genHtml += "</div>";
  D6.genHtml = genHtml;
